...
def maj_distance():
    """Lit la distance depuis l'Arduino et met à jour l'affichage"""
    if arduino and arduino.is_open:
        try:
            ligne = arduino.readline().decode('utf-8').strip()
            if ligne:
                try:
                    d = float(ligne)  # Arduino envoie la distance en cm
                    distanceAffiche.config(text=f"{d:.1f} cm")
                except ValueError:
                    distanceAffiche.config(text="Invalide")
        except:
            distanceAffiche.config(text="Erreur Série")
    else:
        distanceAffiche.config(text="Arduino non connecté")

    ultraGru.after(500, maj_distance)  # rafraîchit toutes les 0.5s
